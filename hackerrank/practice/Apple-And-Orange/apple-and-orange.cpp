...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
  uint amountOfApples{0};
  uint amountOfOranges{0};
  
  // Apples 
  for(size_t i = 0 ; i < apples.size(); i++){
    int land = apples.at(i);
    if( land >= 0 && ( a + land ) >= s  && ( a+land )<= t ){
      amountOfApples++;
    }
  }

  // Oranges
  for(size_t i = 0 ; i < oranges.size(); i++){
    int land = oranges.at(i);
    if( land <= 0 && ( b + land ) >= s  && ( b+land )<= t ){
      amountOfOranges++;
    }
  }

  // Display results
  std::cout << amountOfApples << std::endl;
  std::cout << amountOfOranges << std::endl;
}

int main()
{
  countApplesAndOranges( 7, 8, 7, 9,
  {-1,1,2},
  {-1,1}
  );
